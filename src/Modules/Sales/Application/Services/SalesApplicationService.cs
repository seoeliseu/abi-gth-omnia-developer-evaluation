...
namespace Ambev.DeveloperEvaluation.Sales.Application.Services;
...
public sealed class SalesApplicationService : ISalesApplicationService
{
    ...
    public async Task<Result<SaleDetail>> RemoverAsync(Guid saleId, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        var detalhe = MapearDetalhe(sale);
        await _saleRepository.RemoverAsync(sale, cancellationToken);
        return Result<SaleDetail>.Success(detalhe);
    }
