...
public sealed class Result<T> : Result
{
    ...
    public static Result<T> Failure(ResultErrorType errorType, IEnumerable<ResultError> errors, int? statusCode = null)
    {
    ...
    public static Result<T> Validation(IEnumerable<ResultError> errors, int? statusCode = 400)
        => Failure(ResultErrorType.Validation, errors, statusCode);

    public static Result<T> BusinessRule(IEnumerable<ResultError> errors, int? statusCode = 422)
        => Failure(ResultErrorType.BusinessRule, errors, statusCode);

    public static Result<T> NotFound(IEnumerable<ResultError> errors, int? statusCode = 404)
        => Failure(ResultErrorType.NotFound, errors, statusCode);

    public static Result<T> Conflict(IEnumerable<ResultError> errors, int? statusCode = 409)
        => Failure(ResultErrorType.Conflict, errors, statusCode);

    public static Result<T> Unauthorized(IEnumerable<ResultError> errors, int? statusCode = 401)
        => Failure(ResultErrorType.Unauthorized, errors, statusCode);

    public static Result<T> Forbidden(IEnumerable<ResultError> errors, int? statusCode = 403)
        => Failure(ResultErrorType.Forbidden, errors, statusCode);

    public static Result<T> Unexpected(IEnumerable<ResultError> errors, int? statusCode = 500)
